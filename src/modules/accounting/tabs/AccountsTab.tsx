...
function AccountForm({ onClose }: { onClose: () => void }) {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors, isSubmitting } } = useForm<Form>({
    ...
    defaultValues: { account_type: 'asset', is_group: false },
  })
    ...
    <>
      <div className="form-grid">
        ...
        <div>
          ...
          <input className="erp-input" placeholder="e.g. 1001" {...register('account_code')} />
        ...
        <div>
          ...
          <select className="erp-input" {...register('account_type')}>
...
export default function AccountsTab() {
  ...
  const { data, isLoading } = useAccounts({ account_type: typeFilter || undefined })
