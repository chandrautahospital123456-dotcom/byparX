...
import { useSuppliers, useCreateSupplier, useUpdateParty, useDeleteParty, usePartyLedger } from '@/hooks/useQuery'
import { Button, Modal, Pagination, SkeletonRows, Empty, SearchInput, ConfirmDialog } from '@/components/ui'
...
function SupplierForm({ initial, onClose, onCreate }: { initial?: Party | null; onClose: () => void; onCreate: (d: Form) => Promise<void> }) {
  ...
  const { register, handleSubmit, formState: { isSubmitting } } = useForm<Form>({
    ...
    defaultValues: initial ? {
      ...
      credit_days: initial.credit_days || 30, opening_balance: initial.opening_balance,
  ...
  return (
    <>
      <div className="form-grid">
        {[['Name *','name'],['Phone','phone'],['Email','email'],['PAN No','pan_no'],['Address','address']].map(([l,n]) => (
          <div key={n} style={n === 'address' ? { gridColumn: 'span 2' } : undefined}>
      ...
      <div className="flex justify-end gap-2 mt-5 pt-4 border-t border-[var(--border)]">
        ...
        <Button variant="primary" loading={isSubmitting} onClick={onSubmit}>{initial ? 'Save' : 'Create Supplier'}</Button>
      </div>
    </>
  )
}
  ...
  return (
    <div>
      <div className="page-header">
        <div><div className="page-breadcrumb">Parties</div><h1 className="page-title">Suppliers</h1></div>
        <Button variant="primary" icon={<Plus size={14}/>} onClick={() => { setEditing(null); setModal(true) }}>New Supplier</Button>
      </div>
      ...
      <div className="table-card">
        <div className="overflow-x-auto">
          <table className="erp-table">
            <thead><tr><th>Code</th><th>Name</th><th>Phone</th><th>PAN</th><th className="td-right">Balance</th><th></th></tr></thead>
            <tbody>
              {isLoading
                ? <SkeletonRows cols={6} />
                : rows.length
                  ? rows.map(s => (
                      <tr key={s.id}>
                        <td className="td-mono text-brand">{s.code}</td>
                        <td className="font-semibold">{s.name}</td>
                        <td>{s.phone || '—'}</td>
                        <td className="td-mono">{s.pan_no || '—'}</td>
                        <td className={`td-right font-semibold ${Number(s.current_balance) > 0 ? 'text-amber-600' : ''}`}>{fmt(s.current_balance)}</td>
                        <td>
                          <div className="flex gap-1">
                            <Button variant="ghost" size="sm" onClick={() => setLedger(s)}>Ledger</Button>
                            <Button variant="ghost" size="sm" onClick={() => { setEditing(s); setModal(true) }}>Edit</Button>
                            <Button variant="danger" size="sm" onClick={() => setDelId(s.id)}>Del</Button>
                          </div>
                        </td>
                      </tr>
                    ))
                  : <tr><td colSpan={6}><Empty message="No suppliers found"/></td></tr>
              }
            </tbody>
          </table>
        </div>
      ...
      <Modal open={modal} onClose={() => { setModal(false); setEditing(null) }}
        title={editing ? 'Edit Supplier' : 'New Supplier'} size="lg">
        <SupplierForm initial={editing} onClose={() => { setModal(false); setEditing(null) }}
      ...
      <ConfirmDialog open={!!delId} onClose={() => setDelId(null)}
        ...
        title="Delete Supplier" message="Delete this supplier permanently?" danger />
    </div>
  )
}
            ...
            <tbody>
              {isLoading
                ...
                : rows.length
                  ...
                  : <tr><td colSpan={6}><Empty message="No entries"/></td></tr>
              }
            </tbody>
          </table>
        </div>
